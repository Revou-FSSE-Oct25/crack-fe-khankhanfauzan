...
function CalendarMonth({
  ...
  const monthName = new Date(viewYear, viewMonth, 1).toLocaleString("id-ID", {
    month: "long",
    year: "numeric",
  });
    ...
    <Card className={cn("shadow-none", className)}>
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-7 gap-1 sm:gap-2">
          {grid.map((cell, idx) => {
            if (!cell.date) {
              return <div key={idx} className="h-12 sm:h-14 md:h-16 rounded-lg border bg-muted/30" />;
            }
            ...
            return (
              <button
                key={iso}
                className={cn(
                  "h-12 sm:h-14 md:h-16 rounded-lg border p-2 text-left flex flex-col justify-between transition",
                  isSelected ? "border-primary ring-2 ring-primary/20" : "hover:border-input"
                )}
              ...
              >
                ...
                <div className="flex gap-1">
                  {evs.slice(0, 3).map((ev, i) => (
                    <span
                      key={i}
                      className={cn("size-1.5 rounded-full", dotClassByVariant(ev.variant))}
                    />
                  ))}
                  {evs.length > 3 && (
                    <span className="text-[10px] text-muted-foreground">+{evs.length - 3}</span>
                  )}
                </div>
              </button>
            );
